...
public class ProductController {
    ...
    public String toAllProduct(ModelMap map, @PathVariable("state") Integer state) {
        ...
        if (state == 1) {
            ...
            map.put("specialList", productRepostitory.findProductsBySerialNumber("1"));
        } else if (state == 2) {
    ...
    @RequestMapping(value = "/special/buy.html/{id}")
    public String toSpecialBuy(ModelMap map, @PathVariable("id") Integer id) {
        map.put("product", productRepostitory.findProductById(id));
        return "product/special-buy";
    }
    ...
    @RequestMapping(value = "/special/buy.json")
    @ResponseBody
    public Data doSpecialBuy(Data data, Order order,@Param("price") Integer price, @Param("pid") Integer pid) {
        Product productById = productRepostitory.findProductById(pid);
        order.setPid(pid);
        try {
            if (productById.getState()==1) {//其他材料
                order.setName(productById.getName());
                order.setPrices(Integer.valueOf(order.getAttention()) * price);
                order.setHavePay(price.toString());
                order.setOrderNumber(sn.format(new Date()));
                order.setCreateTime(sf.format(new Date()));
                order.setAttention(order.getAttention()+"个");
            } else if (productById.getState()==2) {//门套
                Double sq = (Double.valueOf(order.getHeight()) / 100) *2+((Double.valueOf(order.getWidth()) / 100));
                order.setName(productById.getName());
                order.setSquare(df.format(sq));
                order.setPrices(sq * price);
                order.setHavePay(price.toString());
                order.setOrderNumber(sn.format(new Date()));
                order.setCreateTime(sf.format(new Date()));
                //order.setAttention(order.getAttention()+"米");
            } else if (productById.getState()==3) {//玻璃
                BigDecimal sq = new BigDecimal(order.getHeight()).divide(new BigDecimal(100)).multiply(new BigDecimal(order.getWidth()).divide(new BigDecimal(100)));
                order.setName(productById.getName());
                order.setSquare(df.format(sq));
                order.setPrices(sq.multiply(new BigDecimal(price)).multiply(new BigDecimal(order.getAttention())).doubleValue());
                order.setHavePay(price.toString());
                order.setOrderNumber(sn.format(new Date()));
                order.setCreateTime(sf.format(new Date()));
                order.setAttention(order.getAttention()+"块");
            }
            orderRepostitory.save(order);
            data.setSuccess(true);
            data.setMsg("下单成功");
